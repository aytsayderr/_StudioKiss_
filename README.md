...
<head>
    ...
    <style>
        * {
        ...
        .service-item:hover, .interior-item:hover {
...
<footer>
    ...
    <div class="container" style="text-align:center; font-size:0.75rem; margin-top:20px;">
        © StudioKiss 2026 — Организация мероприятий и дизайн-студия
    </div>
</footer>

<script>
    // Все пути к изображениям теперь с префиксом "image/"
    const galleryImages = [
        "image/vecher.jpg",
        "image/svadiba.jpg",
        "image/korparat.jpg"
    ...
    function renderHome() {
        app.innerHTML = `
            ...
            <section>
                <div style="display:flex; gap:20px; flex-wrap:wrap; background:#eef3fc; padding:24px; justify-content:center; border:1px solid #cbdff2; border-radius:8px;">
                    <a data-page="about" class="btn-more" style="background:#fff;">О нас</a>
                    <a data-page="servicesPage" class="btn-more" style="background:#fff;">Наши услуги</a>
                    <a data-page="contacts" class="btn-more" style="background:#fff;">Контакты</a>
                </div>
            </section>
            <section>
                ...
                <div class="interior-grid">
                    ${interiorStudios.map(st => `
                        <div class="interior-item">
                            <div style="background-image: url('${st.image}'); background-size: cover; background-position: center; height:160px; margin-bottom:16px; border-radius:4px;">
                            </div>
                            <h3>${st.title}</h3>
                            <p>${st.desc}</p>
                            <div class="price">${st.price}</div>
                            <div style="font-size:0.85rem; color:#567d9e; margin-top:8px;"> ${st.features}</div>
    ...
    function renderInteriorsFull() {
        app.innerHTML = `
            <div class="page-card">
                <h2 class="section-title">Интерьерные студии</h2>
                <p style="margin-bottom:24px;">Аренда стильных пространств для ваших мероприятий, съёмок и встреч</p>
                <div class="interior-grid">
                    ${interiorStudios.map(st => `
                        <div class="interior-item">
                            <div style="background-image: url('${st.image}'); background-size: cover; background-position: center; height:200px; margin-bottom:16px; border-radius:4px;">
                            </div>
                            <h3>${st.title}</h3>
                            <p>${st.desc}</p>
                            <div class="price">${st.price}</div>
                            <div style="font-size:0.85rem; color:#567d9e; margin-top:8px;">📐 ${st.features}</div>
                            <a data-page="orderQuick" class="btn-more" style="margin-top:16px;">Забронировать студию</a>
                        </div>
                    `).join('')}
                </div>
            </div>
    ...
    function renderContacts() {
        app.innerHTML = `<div class="page-card"><h2 class="section-title">Контакты</h2><p>Адрес: Санкт-Петербург, Российский проспект 10, офис 405</p><p>Телефон: +7 (812) 513-67-67, +7 (921) 513-67-67</p><p>E-mail: hello@studiokiss.ru, booking@studiokiss.ru</p><p> Режим работы: ежедневно 9:00 – 20:00</p><hr><iframe width="100%" height="250" style="border:1px solid #cbdff2; background:#f0f0f0; border-radius:8px;" title="Карта проезда" src="https://maps.google.com/maps?q=Санкт-Петербург%20Российский%20проспект%2010&t=&z=15&ie=UTF8&iwloc=&output=embed"></iframe></div>`;
    }
